...
meteo() {
	...
	echo -n "\"full_text\":\" $(~/.config/i3status/i3status/meteo.py) \","
	echo -n "\"background\":\"$bg\","
	common
	echo -n "},"
}
...
do
	...
	if [[ $line == *"name"*"id_vpn"* ]]; then
		alacritty -e ~/.config/i3status/i3status/click_vpn.sh &

	# CHECK UPDATES
	elif [[ $line == *"name"*"id_systemupdate"* ]]; then
		alacritty -e ~/.config/i3status/i3status/click_checkupdates.sh &
	...
	# TIME
		...
		alacritty -e ~/.config/i3status/i3status/click_time.sh &
